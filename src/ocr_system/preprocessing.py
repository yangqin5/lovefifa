...
def remove_table_lines(binary_image: np.ndarray) -> np.ndarray:
    """ลบเส้นตารางแนวตั้งเพื่อป้องกันไม่ให้ OCR อ่านตัวอักษรขอบซ้ายผิดพลาด"""
    # สร้าง Kernel สำหรับตรวจจับเส้นแนวตั้ง
    vertical_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 30))
    detected_lines = cv2.morphologyEx(binary_image, cv2.MORPH_OPEN, vertical_kernel, iterations=2)
    
    # ลบเส้นแนวตั้งออกจากภาพ Binary
