...
def evaluate_from_files(ground_truth_json, prediction_json, output_json_path=None, plan_key=None):
    """
    plan_key: ถ้า extracted_result.json เป็นรูปแบบหลายแผน (มี key "plans": {"coop": {...}, "no_coop": {...}})
              ให้ระบุ plan_key เป็น "coop" หรือ "no_coop" เพื่อดึง courses จากแผนนั้นมาประเมิน
              ถ้า None จะอ่านจาก top-level "courses" ตามรูปแบบเดิม (ใช้กับหลักสูตรที่มีแผนเดียว)
    """
    ...
    if plan_key:
        pred_data = pred_json["plans"][plan_key]["courses"]
    else:
        pred_data = pred_json["courses"]
    ...
    # --- คำนวณ Category Level แบบเพื่อน (หาค่าเฉลี่ยจากทุก Page Level รวมกัน) ---
    page_level_raw = {}
    for k, v in metrics["page_level"].items():
        page_level_raw[k] = {
            "em": sum(v["em"]) / len(v["em"]) if len(v["em"]) > 0 else 0,
            "cer": sum(v["cer"]) / len(v["cer"]) if len(v["cer"]) > 0 else 0,
            "wer": sum(v["wer"]) / len(v["wer"]) if len(v["wer"]) > 0 else 0,
        }

    if len(page_level_raw) > 0:
        doc_em = sum(p["em"] for p in page_level_raw.values()) / len(page_level_raw)
        doc_cer = sum(p["cer"] for p in page_level_raw.values()) / len(page_level_raw)
        doc_wer = sum(p["wer"] for p in page_level_raw.values()) / len(page_level_raw)
    else:
        doc_em = doc_cer = doc_wer = 0.0

    category_level = {
        "Exact_Match": format_exact_match(doc_em),
        "CER": format_percent(doc_cer),
        "WER": format_percent(doc_wer)
    }
